...
// Helper function to normalize mileage text for consistency
function normalizeMileageText(mileageText, convertFn) {
  if (!mileageText || typeof mileageText !== 'string') {
    return '';
  }
  
  // If already has both miles and km, return as is
  if (mileageText.includes('miles') && mileageText.includes('km')) {
    return mileageText;
  }
  
  // Add "miles" if missing and convert to include km
  let normalizedText = mileageText;
  if (!mileageText.includes('mile')) {
    normalizedText = mileageText + ' miles';
  }
            ...
            <div className="comparison-table-container">
              <table className="comparison-table">
                <thead>
                  <tr>
                    ...
                    {vehicles.map(vehicle => (
                      <th key={vehicle.id} style={{ maxWidth: '200px', wordWrap: 'break-word' }}>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{t('comparison.mileage', 'Mileage (mi)')}</td>
                    {vehicles.map(vehicle => (
                      <td key={vehicle.id}>
                  ...
                  <tr>
                    <td>{t('comparison.mileageKm', 'Mileage (km)')}</td>
                    {vehicles.map(vehicle => (
                      <td key={vehicle.id}>
                        {vehicle.mileage ? milesToKilometers(vehicle.mileage).toLocaleString() : '0'} {t('comparison.kilometers', 'km')}
                      </td>
                    ))}
                  </tr>
                                    ...
                                    {issue.mileage && (
                                      <div className="issue-detail">
                                        {typeof issue.mileage === 'string' 
                                          ? normalizeMileageText(issue.mileage, milesToKilometers)
                                          : issue.mileage
                                        }
                                      </div>
                                    )}
          ...
          {vehicles.length > 0 && vehicles.some(v => v.reliability_data?.isPremium) && (
            <div className="export-section">
              <button className="export-button" onClick={handleExportPdf} disabled={exporting}>
                {exporting ? t('comparison.generating', 'Generating PDF...') : t('comparison.exportPdf', 'Export Comparison as PDF')}
              </button>
