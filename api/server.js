...
// ADMIN ROUTES - Protected by special admin password
...
// Admin login to verify password
app.post('/api/admin/auth', (req, res) => {
  const { password } = req.body;
  const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'CarReliability2025Admin!';
  
  if (password === ADMIN_PASSWORD) {
    // Generate admin token (short lived)
  ...
  try {
    const result = await pool.query(`
      SELECT 
        ...
        up.recall_alerts,
        up.marketing_emails,
        COUNT(sv.id) as saved_vehicles_count,
        COUNT(s.id) as searches_count
      FROM users u
...
app.post('/api/admin/users/:userId/update', authenticateAdmin, async (req, res) => {
...
app.post('/api/admin/users/:userId/delete', authenticateAdmin, async (req, res) => {
  const { userId } = req.params;
...
app.post('/api/admin/users/:userId/activity', authenticateAdmin, async (req, res) => {
  const { userId } = req.params;
